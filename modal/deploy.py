...
tvmImageCpu = baseImage.run_commands('pip install apache-tvm --pre')
...
@app.function(image=tvmImageCpu, container_idle_timeout=2, cpu=1)
@web_endpoint()
def cpu_1():
  return tuneModel('package/test.onnx', 'llvm', 'cpu')

@app.function(image=tvmImageGpu, container_idle_timeout=2, cpu=1, gpu='t4')
@web_endpoint()
def t4_1():
  return tuneModel('package/test.onnx', 'llvm', 'cpu')
